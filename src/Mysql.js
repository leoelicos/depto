...
const { quaternary } = require('./utils/chalkRender');
class Mysql {
	getDepartments = () =>
		new Promise(function (resolve, reject) {
			...
			db.query(sql, (err, result) => {
				if (err) {
					reject(`Check your SQL: ${sql}\n${err}`);
				} else if (result.length === 0) {
					reject(''.padStart(4) + quaternary('No departments found'));
				} else {
					resolve(result);
				}
			});
	...
	getEmployeesByDepartment = (department_id, departmentName) =>
		new Promise(function (resolve, reject) {
			...
			db.query(sql, params, (err, result) => {
				if (err) reject(new Error(`Check your SQL: ${err}`));
				if (result.length === 0) reject([`No employees found in department ${departmentName}`]);
				resolve(result);
			});
		});
